...
def freight_page():
    ...
    with col1:
        if st.button("Enviar"):
            ...
            or len(st.session_state["currency_values"]) < st.session_state["tax_quantity"]  # noqa: E501
            ):
                st.error(
            ...
            elif any(tax == 0 or tax is None for tax in st.session_state["tax_values"]) or any(  # noqa: E501
            currency is None for currency in st.session_state["currency_values"]
            ):
                st.error("Preencha todos os valores e moedas para as taxas antes de enviar.")  # noqa: E501
            else:
                json_data = {
                    "house_bl": hbl_freight,
                    "master_bl": mbl_freight,
